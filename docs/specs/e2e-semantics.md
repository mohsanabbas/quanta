...
| NackAware sink delivery failure  | `nackAll` → `coord.Nack` → barrier aborted → DLQ publish → commit.                                                                                    | **Yes (via DLQ)**| NackAware sinks (Kafka, S3) route to engine DLQ. See [Error Handling](error-handling.md). |
| AckAware-only sink failure       | ackLoop withholds ack → barrier stays `Live` → offset never committed.                                                                                 | **No**           | Legacy path. Source redelivers on restart.                                            |
